"use strict";
! function () {
    var e = {
        isLine: !1,
        isArrow: !1,
        isArc: !1,
        isDragLastPath: !1,
        isDragAllPaths: !1,
        isRectangle: !1,
        isQuadraticCurve: !1,
        isBezierCurve: !1,
        isPencil: !1,
        isMarker: !0,
        isEraser: !1,
        isText: !1,
        isImage: !1,
        isPdf: !1,
        isSnap: !1,
        isClear: !1,
        set: function (e) {
            this.isLine = this.isArrow = this.isArc = this.isDragLastPath = this.isDragAllPaths = this.isRectangle = this.isQuadraticCurve = this.isBezierCurve = this.isPencil = this.isMarker = this.isEraser = this.isText = this.isImage = this.isPdf = this.snap = this.clear = !1, this["is" + e] = !0
        }
    };

    function t(e, n, o) {
        if (!(n.split(" ").length > 1)) return e.addEventListener ? (e.addEventListener(n, o, !1), !0) : e.attachEvent ? e.attachEvent("on" + n, o) : (e["on" + n] = o, this);
        for (var A = n.split(" "), i = 0; i < A.length; i++) t(e, A[i], o)
    }

    function n(e) {
        return document.getElementById(e)
    }
    var o = [],
        A = n("code-text"),
        i = 2,
        a = "#6c96c8",
        s = "rgba(0,0,0,0)",
        r = 1,
        l = "source-over",
        g = "round",
        c = '15px "Arial"',
        d = "round";

    function f(e) {
        var t = n(e),
            o = t.getContext("2d");
        return t.setAttribute("width", innerWidth), t.setAttribute("height", innerHeight), o.lineWidth = i, o.strokeStyle = a, o.fillStyle = s, o.font = c, o
    }
    var p = f("main-canvas"),
        h = f("temp-canvas"),
        u = {
            updateTextArea: function () {
                var e = u,
                    t = e.toFixed,
                    o = e.getPoint,
                    A = n("is-absolute-points").checked,
                    i = n("is-shorten-code").checked;
                A && i && e.absoluteShortened(), A && !i && e.absoluteNOTShortened(t), !A && i && e.relativeShortened(t, o), A || i || e.relativeNOTShortened(t, o)
            },
            toFixed: function (e) {
                return Number(e).toFixed(1)
            },
            getPoint: function (e, t, n) {
                return e = e > t ? n + " + " + (e - t) : e < t ? n + " - " + (t - e) : n
            },
            absoluteShortened: function () {
                for (var e, t = "", n = o.length, i = 0; i < n; i++) e = o[i], t += this.shortenHelper(e[0], e[1], e[2]);
                t = t.substr(0, t.length - 2), A.value = "var points = [" + t + "], length = points.length, point, p, i = 0;\n\n" + w.toString() + "\n\n" + this.forLoop, this.prevProps = null
            },
            absoluteNOTShortened: function (e) {
                var t, n, i, a = [];
                for (t = 0; t < o.length; t++) n = (i = o[t])[1], "pencil" === i[0] && (a[t] = ["context.beginPath();\ncontext.moveTo(" + n[0] + ", " + n[1] + ");\ncontext.lineTo(" + n[2] + ", " + n[3] + ");\n" + this.strokeOrFill(i[2])]), "marker" === i[0] && (a[t] = ["context.beginPath();\ncontext.moveTo(" + n[0] + ", " + n[1] + ");\ncontext.lineTo(" + n[2] + ", " + n[3] + ");\n" + this.strokeOrFill(i[2])]), "eraser" === i[0] && (a[t] = ["context.beginPath();\ncontext.moveTo(" + n[0] + ", " + n[1] + ");\ncontext.lineTo(" + n[2] + ", " + n[3] + ");\n" + this.strokeOrFill(i[2])]), "line" === i[0] && (a[t] = ["context.beginPath();\ncontext.moveTo(" + n[0] + ", " + n[1] + ");\ncontext.lineTo(" + n[2] + ", " + n[3] + ");\n" + this.strokeOrFill(i[2])]), "pencil" === i[0] && (a[t] = ["context.beginPath();\ncontext.moveTo(" + n[0] + ", " + n[1] + ");\ncontext.lineTo(" + n[2] + ", " + n[3] + ");\n" + this.strokeOrFill(i[2])]), "text" === i[0] && (a[t] = [this.strokeOrFill(i[2]) + "\ncontext.fillText(" + n[0] + ", " + n[1] + ", " + n[2] + ");"]), "arrow" === i[0] && (a[t] = ["drawArrow(" + n[0] + ", " + n[1] + ", " + n[2] + ", " + n[3] + ", '" + i[2].join("','") + "');"]), "arc" === i[0] && (a[t] = ["context.beginPath(); \ncontext.arc(" + e(n[0]) + "," + e(n[1]) + "," + e(n[2]) + "," + e(n[3]) + ", 0," + n[4] + "); \n" + this.strokeOrFill(i[2])]), "rect" === i[0] && (a[t] = [this.strokeOrFill(i[2]) + "\ncontext.strokeRect(" + n[0] + ", " + n[1] + "," + n[2] + "," + n[3] + ");\ncontext.fillRect(" + n[0] + ", " + n[1] + "," + n[2] + "," + n[3] + ");"]), "quadratic" === i[0] && (a[t] = ["context.beginPath();\ncontext.moveTo(" + n[0] + ", " + n[1] + ");\ncontext.quadraticCurveTo(" + n[2] + ", " + n[3] + ", " + n[4] + ", " + n[5] + ");\n" + this.strokeOrFill(i[2])]), "bezier" === i[0] && (a[t] = ["context.beginPath();\ncontext.moveTo(" + n[0] + ", " + n[1] + ");\ncontext.bezierCurveTo(" + n[2] + ", " + n[3] + ", " + n[4] + ", " + n[5] + ", " + n[6] + ", " + n[7] + ");\n" + this.strokeOrFill(i[2])]);
                A.value = a.join("\n\n") + this.strokeFillText + "\n\n" + w.toString(), this.prevProps = null
            },
            relativeShortened: function (e, t) {
                for (var n, i, a = 0, s = o.length, r = "", l = 0, g = 0; a < s; a++) n = (i = o[a])[1], 0 === a && (l = n[0], g = n[1]), "text" === i[0] && (l = n[1], g = n[2]), "pencil" === i[0] && (r += this.shortenHelper(i[0], [t(n[0], l, "x"), t(n[1], g, "y"), t(n[2], l, "x"), t(n[3], g, "y")], i[2])), "marker" === i[0] && (r += this.shortenHelper(i[0], [t(n[0], l, "x"), t(n[1], g, "y"), t(n[2], l, "x"), t(n[3], g, "y")], i[2])), "eraser" === i[0] && (r += this.shortenHelper(i[0], [t(n[0], l, "x"), t(n[1], g, "y"), t(n[2], l, "x"), t(n[3], g, "y")], i[2])), "line" === i[0] && (r += this.shortenHelper(i[0], [t(n[0], l, "x"), t(n[1], g, "y"), t(n[2], l, "x"), t(n[3], g, "y")], i[2])), "pencil" === i[0] && (r += this.shortenHelper(i[0], [t(n[0], l, "x"), t(n[1], g, "y"), t(n[2], l, "x"), t(n[3], g, "y")], i[2])), "arrow" === i[0] && (r += this.shortenHelper(i[0], [t(n[0], l, "x"), t(n[1], g, "y"), t(n[2], l, "x"), t(n[3], g, "y")], i[2])), "text" === i[0] && (r += this.shortenHelper(i[0], [n[0], t(n[1], l, "x"), t(n[2], g, "y")], i[2])), "arc" === i[0] && (r += this.shortenHelper(i[0], [t(n[0], l, "x"), t(n[1], g, "y"), n[2], n[3], n[4]], i[2])), "rect" === i[0] && (r += this.shortenHelper(i[0], [t(n[0], l, "x"), t(n[1], g, "y"), t(n[2], l, "x"), t(n[3], g, "y")], i[2])), "quadratic" === i[0] && (r += this.shortenHelper(i[0], [t(n[0], l, "x"), t(n[1], g, "y"), t(n[2], l, "x"), t(n[3], g, "y"), t(n[4], l, "x"), t(n[5], g, "y")], i[2])), "bezier" === i[0] && (r += this.shortenHelper(i[0], [t(n[0], l, "x"), t(n[1], g, "y"), t(n[2], l, "x"), t(n[3], g, "y"), t(n[4], l, "x"), t(n[5], g, "y"), t(n[6], l, "x"), t(n[7], g, "y")], i[2]));
                r = r.substr(0, r.length - 2), A.value = "var x = " + l + ", y = " + g + ", points = [" + r + "], length = points.length, point, p, i = 0;\n\n" + w.toString() + "\n\n" + this.forLoop, this.prevProps = null
            },
            relativeNOTShortened: function (e, t) {
                var n, i, a, s = o.length,
                    r = "",
                    l = 0,
                    g = 0;
                for (n = 0; n < s; n++) i = (a = o[n])[1], 0 === n && (l = i[0], g = i[1], "text" === a[0] && (l = i[1], g = i[2]), r = "var x = " + l + ", y = " + g + ";\n\n"), "arc" === a[0] && (r += "context.beginPath();\ncontext.arc(" + t(i[0], l, "x") + ", " + t(i[1], g, "y") + ", " + i[2] + ", " + i[3] + ", 0, " + i[4] + ");\n" + this.strokeOrFill(a[2])), "pencil" === a[0] && (r += "context.beginPath();\ncontext.moveTo(" + t(i[0], l, "x") + ", " + t(i[1], g, "y") + ");\ncontext.lineTo(" + t(i[2], l, "x") + ", " + t(i[3], g, "y") + ");\n" + this.strokeOrFill(a[2])), "marker" === a[0] && (r += "context.beginPath();\ncontext.moveTo(" + t(i[0], l, "x") + ", " + t(i[1], g, "y") + ");\ncontext.lineTo(" + t(i[2], l, "x") + ", " + t(i[3], g, "y") + ");\n" + this.strokeOrFill(a[2])), "eraser" === a[0] && (r += "context.beginPath();\ncontext.moveTo(" + t(i[0], l, "x") + ", " + t(i[1], g, "y") + ");\ncontext.lineTo(" + t(i[2], l, "x") + ", " + t(i[3], g, "y") + ");\n" + this.strokeOrFill(a[2])), "line" === a[0] && (r += "context.beginPath();\ncontext.moveTo(" + t(i[0], l, "x") + ", " + t(i[1], g, "y") + ");\ncontext.lineTo(" + t(i[2], l, "x") + ", " + t(i[3], g, "y") + ");\n" + this.strokeOrFill(a[2])), "pencil" === a[0] && (r += "context.beginPath();\ncontext.moveTo(" + t(i[0], l, "x") + ", " + t(i[1], g, "y") + ");\ncontext.lineTo(" + t(i[2], l, "x") + ", " + t(i[3], g, "y") + ");\n" + this.strokeOrFill(a[2])), "arrow" === a[0] && (r += "drawArrow(" + t(i[0], l, "x") + ", " + t(i[1], g, "y") + ", " + t(i[2], l, "x") + ", " + t(i[3], g, "y") + ", '" + a[2].join("','") + "');\n"), "text" === a[0] && (r += this.strokeOrFill(a[2]) + "\ncontext.fillText(" + i[0] + ", " + t(i[1], l, "x") + ", " + t(i[2], g, "y") + ");"), "rect" === a[0] && (r += this.strokeOrFill(a[2]) + "\ncontext.strokeRect(" + t(i[0], l, "x") + ", " + t(i[1], g, "y") + ", " + t(i[2], l, "x") + ", " + t(i[3], g, "y") + ");\ncontext.fillRect(" + t(i[0], l, "x") + ", " + t(i[1], g, "y") + ", " + t(i[2], l, "x") + ", " + t(i[3], g, "y") + ");"), "quadratic" === a[0] && (r += "context.beginPath();\ncontext.moveTo(" + t(i[0], l, "x") + ", " + t(i[1], g, "y") + ");\ncontext.quadraticCurveTo(" + t(i[2], l, "x") + ", " + t(i[3], g, "y") + ", " + t(i[4], l, "x") + ", " + t(i[5], g, "y") + ");\n" + this.strokeOrFill(a[2])), "bezier" === a[0] && (r += "context.beginPath();\ncontext.moveTo(" + t(i[0], l, "x") + ", " + t(i[1], g, "y") + ");\ncontext.bezierCurveTo(" + t(i[2], l, "x") + ", " + t(i[3], g, "y") + ", " + t(i[4], l, "x") + ", " + t(i[5], g, "y") + ", " + t(i[6], l, "x") + ", " + t(i[7], g, "y") + ");\n" + this.strokeOrFill(a[2])), n !== s - 1 && (r += "\n\n");
                A.value = r + this.strokeFillText + "\n\n" + w.toString(), this.prevProps = null
            },
            forLoop: 'for(i; i < length; i++) {\n    p = points[i];\n    point = p[1];\n    context.beginPath();\n\n    if(p[2]) { \n\tcontext.lineWidth = p[2][0];\n\tcontext.strokeStyle = p[2][1];\n\tcontext.fillStyle = p[2][2];\n\tcontext.globalAlpha = p[2][3];\n\tcontext.globalCompositeOperation = p[2][4];\n\tcontext.lineCap = p[2][5];\n\tcontext.lineJoin = p[2][6];\n\tcontext.font = p[2][7];\n    }\n\n    if(p[0] === "line") { \n\tcontext.moveTo(point[0], point[1]);\n\tcontext.lineTo(point[2], point[3]);\n    }\n\n    if(p[0] === "arrow") { \n\tdrawArrow(point[0], point[1], point[2], point[3], p[2]);\n    }\n\n    if(p[0] === "pencil") { \n\tcontext.moveTo(point[0], point[1]);\n\tcontext.lineTo(point[2], point[3]);\n    }\n\n    if(p[0] === "marker") { \n\tcontext.moveTo(point[0], point[1]);\n\tcontext.lineTo(point[2], point[3]);\n    }\n\n    if(p[0] === "text") { \n\tcontext.fillText(point[0], point[1], point[2]);\n    }\n\n    if(p[0] === "eraser") { \n\tcontext.moveTo(point[0], point[1]);\n\tcontext.lineTo(point[2], point[3]);\n    }\n\n    if(p[0] === "arc") context.arc(point[0], point[1], point[2], point[3], 0, point[4]); \n\n    if(p[0] === "rect") {\n\tcontext.strokeRect(point[0], point[1], point[2], point[3]);\n\tcontext.fillRect(point[0], point[1], point[2], point[3]);\n    }\n\n    if(p[0] === "quadratic") {\n\tcontext.moveTo(point[0], point[1]);\n\tcontext.quadraticCurveTo(point[2], point[3], point[4], point[5]);\n    }\n\n    if(p[0] === "bezier") {\n\tcontext.moveTo(point[0], point[1]);\n\tcontext.bezierCurveTo(point[2], point[3], point[4], point[5], point[6], point[7]);\n    }\n\n    context.stroke();\n    context.fill();\n}',
            strokeFillText: "\n\nfunction strokeOrFill(lineWidth, strokeStyle, fillStyle, globalAlpha, globalCompositeOperation, lineCap, lineJoin, font) { \n    if(lineWidth) { \n\tcontext.globalAlpha = globalAlpha;\n\tcontext.globalCompositeOperation = globalCompositeOperation;\n\tcontext.lineCap = lineCap;\n\tcontext.lineJoin = lineJoin;\n\tcontext.lineWidth = lineWidth;\n\tcontext.strokeStyle = strokeStyle;\n\tcontext.fillStyle = fillStyle;\n\tcontext.font = font;\n    } \n\n    context.stroke();\n    context.fill();\n}",
            strokeOrFill: function (e) {
                return this.prevProps && this.prevProps === e.join(",") ? "strokeOrFill();" : (this.prevProps = e.join(","), "strokeOrFill('" + e.join("', '") + "');")
            },
            prevProps: null,
            shortenHelper: function (e, t, n) {
                var o = "['" + e + "', [" + t.join(", ") + "]";
                return this.prevProps && this.prevProps === n.join(",") || (this.prevProps = n.join(","), o += ", ['" + n.join("', '") + "']"), o + "], "
            }
        };

    function w(e, t, n, o, A) {
        function i(e, t) {
            e = e || function (e) {
                return [(e = e || {}).lineWidth || 2, e.strokeStyle || "#6c96c8", e.fillStyle || "rgba(0,0,0,0)", e.globalAlpha || 1, e.globalCompositeOperation || "source-over", e.lineCap || "round", e.lineJoin || "round", e.font || '15px "Arial"']
            }(), p.globalAlpha = e[3], p.globalCompositeOperation = e[4], p.lineCap = e[5], p.lineJoin = e[6], p.lineWidth = e[0], p.strokeStyle = e[1], p.fillStyle = e[2], p.font = e[7], t || (p.stroke(), p.fill())
        }
        var a = Math.atan2(o - t, n - e);
        p.beginPath(), p.moveTo(e, t), p.lineTo(n, o), i(), p.beginPath(), p.moveTo(n, o), p.lineTo(n - 10 * Math.cos(a - Math.PI / 7), o - 10 * Math.sin(a - Math.PI / 7)), p.lineTo(n - 10 * Math.cos(a + Math.PI / 7), o - 10 * Math.sin(a + Math.PI / 7)), p.lineTo(n, o), p.lineTo(n - 10 * Math.cos(a - Math.PI / 7), o - 10 * Math.sin(a - Math.PI / 7)), i()
    }

    function v() {
        var t = e;
        t.isArc ? K.end() : t.isQuadraticCurve ? J.end() : t.isBezierCurve && N.end(), b.redraw(), X.text && X.text.length && (X.appendPoints(), X.onShapeUnSelected()), X.showOrHideTextTools("hide")
    }
    var C, B = [];

    function I() {
        v(), y.global.startingIndex = 0, n("copy-last").checked ? (B = o[o.length - 1], te(n("drag-last-path"), "DragLastPath")) : (B = o, te(n("drag-all-paths"), "DragAllPaths"))
    }

    function P() {
        v(), y.global.startingIndex = 0, n("copy-last").checked ? (o[o.length] = B, y.global = {
            prevX: 0,
            prevY: 0,
            startingIndex: o.length - 1
        }, y.dragAllPaths(0, 0), te(n("drag-last-path"), "DragLastPath")) : (y.global.startingIndex = o.length, o = o.concat(B), te(n("drag-all-paths"), "DragAllPaths"))
    }

    function m(e) {
        return parseInt(Q(e).substring(0, 2), 16)
    }

    function E(e) {
        return parseInt(Q(e).substring(2, 4), 16)
    }

    function D(e) {
        return parseInt(Q(e).substring(4, 6), 16)
    }

    function Q(e) {
        return "#" == e.charAt(0) ? e.substring(1, 7) : e
    }

    function x(e) {
        if (null === e || "object" != typeof e || "isActiveClone" in e) return e;
        if (e instanceof Date) var t = new e.constructor;
        else t = e.constructor();
        for (var n in e) Object.prototype.hasOwnProperty.call(e, n) && (e.isActiveClone = null, t[n] = x(e[n]), delete e.isActiveClone);
        return t
    }

    function Y(e) {
        return [m(e), E(e), D(e)]
    }
    var b = {
            redraw: function () {
                h.clearRect(0, 0, innerWidth, innerHeight), p.clearRect(0, 0, innerWidth, innerHeight);
                var e, t, n = o.length;
                for (e = 0; e < n; e++)(t = o[e]) && t.length && this[t[0]] && this[t[0]](p, t[1], t[2])
            },
            getOptions: function (e) {
                return [(e = e || {}).lineWidth || i, e.strokeStyle || a, e.fillStyle || s, e.globalAlpha || r, e.globalCompositeOperation || l, e.lineCap || g, e.lineJoin || d, e.font || c]
            },
            handleOptions: function (e, t, n) {
                t = t || this.getOptions(), e.globalAlpha = t[3], e.globalCompositeOperation = t[4], e.lineCap = t[5], e.lineJoin = t[6], e.lineWidth = t[0], e.strokeStyle = t[1], e.fillStyle = t[2], e.font = t[7], n || (e.stroke(), e.fill())
            },
            line: function (e, t, n) {
                e.beginPath(), e.moveTo(t[0], t[1]), e.lineTo(t[2], t[3]), this.handleOptions(e, n)
            },
            pencil: function (e, t, n) {
                e.beginPath(), e.moveTo(t[0], t[1]), e.lineTo(t[2], t[3]), this.handleOptions(e, n)
            },
            marker: function (e, t, n) {
                e.beginPath(), e.moveTo(t[0], t[1]), e.lineTo(t[2], t[3]), this.handleOptions(e, n)
            },
            arrow: function (e, t, n) {
                var o = t[0],
                    A = t[1],
                    a = t[2],
                    s = t[3],
                    r = G.arrowSize;
                10 == r && (r = 5 * (n ? n[0] : i));
                var l = Math.atan2(s - A, a - o);
                e.beginPath(), e.moveTo(o, A), e.lineTo(a, s), this.handleOptions(e, n), e.beginPath(), e.moveTo(a, s), e.lineTo(a - r * Math.cos(l - Math.PI / 7), s - r * Math.sin(l - Math.PI / 7)), e.lineTo(a - r * Math.cos(l + Math.PI / 7), s - r * Math.sin(l + Math.PI / 7)), e.lineTo(a, s), e.lineTo(a - r * Math.cos(l - Math.PI / 7), s - r * Math.sin(l - Math.PI / 7)), this.handleOptions(e, n)
            },
            text: function (e, t, n) {
                this.handleOptions(e, n), e.fillStyle = X.getFillColor(n[2]), e.fillText(t[0].substr(1, t[0].length - 2), t[1], t[2])
            },
            arc: function (e, t, n) {
                e.beginPath(), e.arc(t[0], t[1], t[2], t[3], 0, t[4]), this.handleOptions(e, n)
            },
            rect: function (e, t, n) {
                this.handleOptions(e, n, !0), e.strokeRect(t[0], t[1], t[2], t[3]), e.fillRect(t[0], t[1], t[2], t[3])
            },
            image: function (e, t, n) {
                var o;
                if (this.handleOptions(e, n, !0), !(o = W.images[t[5]])) return (o = new Image).onload = function () {
                    var n = W.images.length;
                    W.lastImageURL = o.src, W.lastImageIndex = n, W.images.push(o), e.drawImage(o, t[1], t[2], t[3], t[4])
                }, void(o.src = t[0]);
                e.drawImage(o, t[1], t[2], t[3], t[4])
            },
            pdf: function (e, t, n) {
                var o;
                if (this.handleOptions(e, n, !0), !(o = Z.images[t[5]])) return (o = new Image).onload = function () {
                    var n = W.images.length;
                    Z.lastPage = o.src, Z.lastIndex = n, Z.images.push(o), e.drawImage(o, t[1], t[2], t[3], t[4])
                }, void(o.src = t[0]);
                e.drawImage(o, t[1], t[2], t[3], t[4]), Z.reset_pos(t[1], t[2])
            },
            quadratic: function (e, t, n) {
                e.beginPath(), e.moveTo(t[0], t[1]), e.quadraticCurveTo(t[2], t[3], t[4], t[5]), this.handleOptions(e, n)
            },
            bezier: function (e, t, n) {
                e.beginPath(), e.moveTo(t[0], t[1]), e.bezierCurveTo(t[2], t[3], t[4], t[5], t[6], t[7]), this.handleOptions(e, n)
            }
        },
        y = {
            global: {
                prevX: 0,
                prevY: 0,
                ismousedown: !1,
                pointsToMove: "all",
                startingIndex: 0
            },
            mousedown: function (e) {
                C && (I(), P(), C = !1);
                var t = y,
                    n = t.global,
                    A = e.pageX - ae.offsetLeft,
                    i = e.pageY - ae.offsetTop;
                if (n.prevX = A, n.prevY = i, n.pointsToMove = "all", o.length) {
                    var a = o[o.length - 1],
                        s = a[1];
                    "line" === a[0] && (t.isPointInPath(A, i, s[0], s[1]) && (n.pointsToMove = "head"), t.isPointInPath(A, i, s[2], s[3]) && (n.pointsToMove = "tail")), "pencil" === a[0] && (t.isPointInPath(A, i, s[0], s[1]) && (n.pointsToMove = "head"), t.isPointInPath(A, i, s[2], s[3]) && (n.pointsToMove = "tail")), "arrow" === a[0] && (t.isPointInPath(A, i, s[0], s[1]) && (n.pointsToMove = "head"), t.isPointInPath(A, i, s[2], s[3]) && (n.pointsToMove = "tail")), "rect" === a[0] && (t.isPointInPath(A, i, s[0], s[1]) && (n.pointsToMove = "stretch-first"), t.isPointInPath(A, i, s[0] + s[2], s[1]) && (n.pointsToMove = "stretch-second"), t.isPointInPath(A, i, s[0], s[1] + s[3]) && (n.pointsToMove = "stretch-third"), t.isPointInPath(A, i, s[0] + s[2], s[1] + s[3]) && (n.pointsToMove = "stretch-last")), "image" === a[0] && (t.isPointInPath(A, i, s[1], s[2]) && (n.pointsToMove = "stretch-first"), t.isPointInPath(A, i, s[1] + s[3], s[2]) && (n.pointsToMove = "stretch-second"), t.isPointInPath(A, i, s[1], s[2] + s[4]) && (n.pointsToMove = "stretch-third"), t.isPointInPath(A, i, s[1] + s[3], s[2] + s[4]) && (n.pointsToMove = "stretch-last")), "pdf" === a[0] && (t.isPointInPath(A, i, s[1], s[2]) && (n.pointsToMove = "stretch-first"), t.isPointInPath(A, i, s[1] + s[3], s[2]) && (n.pointsToMove = "stretch-second"), t.isPointInPath(A, i, s[1], s[2] + s[4]) && (n.pointsToMove = "stretch-third"), t.isPointInPath(A, i, s[1] + s[3], s[2] + s[4]) && (n.pointsToMove = "stretch-last")), "quadratic" === a[0] && (t.isPointInPath(A, i, s[0], s[1]) && (n.pointsToMove = "starting-points"), t.isPointInPath(A, i, s[2], s[3]) && (n.pointsToMove = "control-points"), t.isPointInPath(A, i, s[4], s[5]) && (n.pointsToMove = "ending-points")), "bezier" === a[0] && (t.isPointInPath(A, i, s[0], s[1]) && (n.pointsToMove = "starting-points"), t.isPointInPath(A, i, s[2], s[3]) && (n.pointsToMove = "1st-control-points"), t.isPointInPath(A, i, s[4], s[5]) && (n.pointsToMove = "2nd-control-points"), t.isPointInPath(A, i, s[6], s[7]) && (n.pointsToMove = "ending-points"))
                }
                n.ismousedown = !0
            },
            mouseup: function () {
                var t = this.global;
                e.isDragLastPath && (h.clearRect(0, 0, innerWidth, innerHeight), p.clearRect(0, 0, innerWidth, innerHeight), this.end()), t.ismousedown = !1
            },
            mousemove: function (t) {
                var n = t.pageX - ae.offsetLeft,
                    o = t.pageY - ae.offsetTop,
                    A = this.global;
                b.redraw(), A.ismousedown && this.dragShape(n, o), e.isDragLastPath && this.init()
            },
            init: function () {
                if (o.length) {
                    var e = o[o.length - 1],
                        t = e[1];
                    this.global.ismousedown ? h.fillStyle = "rgba(255,85 ,154,.9)" : h.fillStyle = "rgba(255,85 ,154,.4)", "quadratic" === e[0] && (h.beginPath(), h.arc(t[0], t[1], 10, 2 * Math.PI, 0, !1), h.arc(t[2], t[3], 10, 2 * Math.PI, 0, !1), h.arc(t[4], t[5], 10, 2 * Math.PI, 0, !1), h.fill()), "bezier" === e[0] && (h.beginPath(), h.arc(t[0], t[1], 10, 2 * Math.PI, 0, !1), h.arc(t[2], t[3], 10, 2 * Math.PI, 0, !1), h.arc(t[4], t[5], 10, 2 * Math.PI, 0, !1), h.arc(t[6], t[7], 10, 2 * Math.PI, 0, !1), h.fill()), "line" === e[0] && (h.beginPath(), h.arc(t[0], t[1], 10, 2 * Math.PI, 0, !1), h.arc(t[2], t[3], 10, 2 * Math.PI, 0, !1), h.fill()), "pencil" === e[0] && (h.beginPath(), h.arc(t[0], t[1], 10, 2 * Math.PI, 0, !1), h.arc(t[2], t[3], 10, 2 * Math.PI, 0, !1), h.fill()), "arrow" === e[0] && (h.beginPath(), h.arc(t[0], t[1], 10, 2 * Math.PI, 0, !1), h.arc(t[2], t[3], 10, 2 * Math.PI, 0, !1), h.fill()), "text" === e[0] && (h.font = "15px Verdana", h.fillText(t[0], t[1], t[2])), "rect" === e[0] && (h.beginPath(), h.arc(t[0], t[1], 10, 2 * Math.PI, 0, !1), h.fill(), h.beginPath(), h.arc(t[0] + t[2], t[1], 10, 2 * Math.PI, 0, !1), h.fill(), h.beginPath(), h.arc(t[0], t[1] + t[3], 10, 2 * Math.PI, 0, !1), h.fill(), h.beginPath(), h.arc(t[0] + t[2], t[1] + t[3], 10, 2 * Math.PI, 0, !1), h.fill()), "image" === e[0] && (h.beginPath(), h.arc(t[1], t[2], 10, 2 * Math.PI, 0, !1), h.fill(), h.beginPath(), h.arc(t[1] + t[3], t[2], 10, 2 * Math.PI, 0, !1), h.fill(), h.beginPath(), h.arc(t[1], t[2] + t[4], 10, 2 * Math.PI, 0, !1), h.fill(), h.beginPath(), h.arc(t[1] + t[3], t[2] + t[4], 10, 2 * Math.PI, 0, !1), h.fill()), "pdf" === e[0] && (h.beginPath(), h.arc(t[1], t[2], 10, 2 * Math.PI, 0, !1), h.fill(), h.beginPath(), h.arc(t[1] + t[3], t[2], 10, 2 * Math.PI, 0, !1), h.fill(), h.beginPath(), h.arc(t[1], t[2] + t[4], 10, 2 * Math.PI, 0, !1), h.fill(), h.beginPath(), h.arc(t[1] + t[3], t[2] + t[4], 10, 2 * Math.PI, 0, !1), h.fill())
                }
            },
            isPointInPath: function (e, t, n, o) {
                return e > n - 10 && e < n + 10 && t > o - 10 && t < o + 10
            },
            getPoint: function (e, t, n) {
                return e = e > t ? n + (e - t) : n - (t - e)
            },
            getXYWidthHeight: function (e, t, n, o, A) {
                return "stretch-first" == A.pointsToMove && (e > n ? (A.x = A.x + (e - n), A.width = A.width - (e - n)) : (A.x = A.x - (n - e), A.width = A.width + (n - e)), t > o ? (A.y = A.y + (t - o), A.height = A.height - (t - o)) : (A.y = A.y - (o - t), A.height = A.height + (o - t))), "stretch-second" == A.pointsToMove && (A.width = e > n ? A.width + (e - n) : A.width - (n - e), t < o ? (A.y = A.y + (t - o), A.height = A.height - (t - o)) : (A.y = A.y - (o - t), A.height = A.height + (o - t))), "stretch-third" == A.pointsToMove && (e > n ? (A.x = A.x + (e - n), A.width = A.width - (e - n)) : (A.x = A.x - (n - e), A.width = A.width + (n - e)), A.height = t < o ? A.height + (t - o) : A.height - (o - t)), A
            },
            dragShape: function (t, n) {
                if (this.global.ismousedown) {
                    h.clearRect(0, 0, innerWidth, innerHeight), e.isDragLastPath && this.dragLastPath(t, n), e.isDragAllPaths && this.dragAllPaths(t, n);
                    var o = this.global;
                    o.prevX = t, o.prevY = n
                }
            },
            end: function () {
                if (o.length) {
                    h.clearRect(0, 0, innerWidth, innerHeight);
                    var e = o[o.length - 1];
                    b[e[0]](p, e[1], e[2])
                }
            },
            dragAllPaths: function (e, t) {
                for (var n, A, i = this.global, a = i.prevX, s = i.prevY, r = o.length, l = this.getPoint, g = i.startingIndex; g < r; g++) A = (n = o[g])[1], "line" === n[0] && (o[g] = [n[0],
                    [l(e, a, A[0]), l(t, s, A[1]), l(e, a, A[2]), l(t, s, A[3])], n[2]
                ]), "pencil" === n[0] && (o[g] = [n[0],
                    [l(e, a, A[0]), l(t, s, A[1]), l(e, a, A[2]), l(t, s, A[3])], n[2]
                ]), "arrow" === n[0] && (o[g] = [n[0],
                    [l(e, a, A[0]), l(t, s, A[1]), l(e, a, A[2]), l(t, s, A[3])], n[2]
                ]), "text" === n[0] && (o[g] = [n[0],
                    [A[0], l(e, a, A[1]), l(t, s, A[2])], n[2]
                ]), "arc" === n[0] && (o[g] = [n[0],
                    [l(e, a, A[0]), l(t, s, A[1]), A[2], A[3], A[4]], n[2]
                ]), "rect" === n[0] && (o[g] = [n[0],
                    [l(e, a, A[0]), l(t, s, A[1]), A[2], A[3]], n[2]
                ]), "image" === n[0] && (o[g] = [n[0],
                    [A[0], l(e, a, A[1]), l(t, s, A[2]), A[3], A[4], A[5]], n[2]
                ]), "pdf" === n[0] && (o[g] = [n[0],
                    [A[0], l(e, a, A[1]), l(t, s, A[2]), A[3], A[4], A[5]], n[2]
                ]), "quadratic" === n[0] && (o[g] = [n[0],
                    [l(e, a, A[0]), l(t, s, A[1]), l(e, a, A[2]), l(t, s, A[3]), l(e, a, A[4]), l(t, s, A[5])], n[2]
                ]), "bezier" === n[0] && (o[g] = [n[0],
                    [l(e, a, A[0]), l(t, s, A[1]), l(e, a, A[2]), l(t, s, A[3]), l(e, a, A[4]), l(t, s, A[5]), l(e, a, A[6]), l(t, s, A[7])], n[2]
                ])
            },
            dragLastPath: function (e, t) {
                if (o[o.length - 1]) {
                    var n = this.global,
                        A = n.prevX,
                        i = n.prevY,
                        a = o[o.length - 1],
                        s = a[1],
                        r = this.getPoint,
                        l = this.getXYWidthHeight,
                        g = "all" === n.pointsToMove;
                    if ("line" === a[0] && (("head" === n.pointsToMove || g) && (s[0] = r(e, A, s[0]), s[1] = r(t, i, s[1])), ("tail" === n.pointsToMove || g) && (s[2] = r(e, A, s[2]), s[3] = r(t, i, s[3])), o[o.length - 1] = [a[0], s, a[2]]), "pencil" === a[0] && (("head" === n.pointsToMove || g) && (s[0] = r(e, A, s[0]), s[1] = r(t, i, s[1])), ("tail" === n.pointsToMove || g) && (s[2] = r(e, A, s[2]), s[3] = r(t, i, s[3])), o[o.length - 1] = [a[0], s, a[2]]), "arrow" === a[0] && (("head" === n.pointsToMove || g) && (s[0] = r(e, A, s[0]), s[1] = r(t, i, s[1])), ("tail" === n.pointsToMove || g) && (s[2] = r(e, A, s[2]), s[3] = r(t, i, s[3])), o[o.length - 1] = [a[0], s, a[2]]), "text" === a[0] && (("head" === n.pointsToMove || g) && (s[1] = r(e, A, s[1]), s[2] = r(t, i, s[2])), o[o.length - 1] = [a[0], s, a[2]]), "arc" === a[0] && (s[0] = r(e, A, s[0]), s[1] = r(t, i, s[1]), o[o.length - 1] = [a[0], s, a[2]]), "rect" === a[0]) {
                        if (g && (s[0] = r(e, A, s[0]), s[1] = r(t, i, s[1])), "stretch-first" === n.pointsToMove) {
                            var c = l(e, t, A, i, {
                                x: s[0],
                                y: s[1],
                                width: s[2],
                                height: s[3],
                                pointsToMove: n.pointsToMove
                            });
                            s[0] = c.x, s[1] = c.y, s[2] = c.width, s[3] = c.height
                        }
                        if ("stretch-second" === n.pointsToMove) {
                            c = l(e, t, A, i, {
                                x: s[0],
                                y: s[1],
                                width: s[2],
                                height: s[3],
                                pointsToMove: n.pointsToMove
                            });
                            s[1] = c.y, s[2] = c.width, s[3] = c.height
                        }
                        if ("stretch-third" === n.pointsToMove) {
                            c = l(e, t, A, i, {
                                x: s[0],
                                y: s[1],
                                width: s[2],
                                height: s[3],
                                pointsToMove: n.pointsToMove
                            });
                            s[0] = c.x, s[2] = c.width, s[3] = c.height
                        }
                        "stretch-last" === n.pointsToMove && (s[2] = r(e, A, s[2]), s[3] = r(t, i, s[3])), o[o.length - 1] = [a[0], s, a[2]]
                    }
                    if ("image" === a[0]) {
                        if (g && (s[1] = r(e, A, s[1]), s[2] = r(t, i, s[2])), "stretch-first" === n.pointsToMove) {
                            c = l(e, t, A, i, {
                                x: s[1],
                                y: s[2],
                                width: s[3],
                                height: s[4],
                                pointsToMove: n.pointsToMove
                            });
                            s[1] = c.x, s[2] = c.y, s[3] = c.width, s[4] = c.height
                        }
                        if ("stretch-second" === n.pointsToMove) {
                            c = l(e, t, A, i, {
                                x: s[1],
                                y: s[2],
                                width: s[3],
                                height: s[4],
                                pointsToMove: n.pointsToMove
                            });
                            s[2] = c.y, s[3] = c.width, s[4] = c.height
                        }
                        if ("stretch-third" === n.pointsToMove) {
                            c = l(e, t, A, i, {
                                x: s[1],
                                y: s[2],
                                width: s[3],
                                height: s[4],
                                pointsToMove: n.pointsToMove
                            });
                            s[1] = c.x, s[3] = c.width, s[4] = c.height
                        }
                        "stretch-last" === n.pointsToMove && (s[3] = r(e, A, s[3]), s[4] = r(t, i, s[4])), o[o.length - 1] = [a[0], s, a[2]]
                    }
                    if ("pdf" === a[0]) {
                        if (g && (s[1] = r(e, A, s[1]), s[2] = r(t, i, s[2])), "stretch-first" === n.pointsToMove) {
                            c = l(e, t, A, i, {
                                x: s[1],
                                y: s[2],
                                width: s[3],
                                height: s[4],
                                pointsToMove: n.pointsToMove
                            });
                            s[1] = c.x, s[2] = c.y, s[3] = c.width, s[4] = c.height
                        }
                        if ("stretch-second" === n.pointsToMove) {
                            c = l(e, t, A, i, {
                                x: s[1],
                                y: s[2],
                                width: s[3],
                                height: s[4],
                                pointsToMove: n.pointsToMove
                            });
                            s[2] = c.y, s[3] = c.width, s[4] = c.height
                        }
                        if ("stretch-third" === n.pointsToMove) {
                            c = l(e, t, A, i, {
                                x: s[1],
                                y: s[2],
                                width: s[3],
                                height: s[4],
                                pointsToMove: n.pointsToMove
                            });
                            s[1] = c.x, s[3] = c.width, s[4] = c.height
                        }
                        "stretch-last" === n.pointsToMove && (s[3] = r(e, A, s[3]), s[4] = r(t, i, s[4])), o[o.length - 1] = [a[0], s, a[2]]
                    }
                    "quadratic" === a[0] && (("starting-points" === n.pointsToMove || g) && (s[0] = r(e, A, s[0]), s[1] = r(t, i, s[1])), ("control-points" === n.pointsToMove || g) && (s[2] = r(e, A, s[2]), s[3] = r(t, i, s[3])), ("ending-points" === n.pointsToMove || g) && (s[4] = r(e, A, s[4]), s[5] = r(t, i, s[5])), o[o.length - 1] = [a[0], s, a[2]]), "bezier" === a[0] && (("starting-points" === n.pointsToMove || g) && (s[0] = r(e, A, s[0]), s[1] = r(t, i, s[1])), ("1st-control-points" === n.pointsToMove || g) && (s[2] = r(e, A, s[2]), s[3] = r(t, i, s[3])), ("2nd-control-points" === n.pointsToMove || g) && (s[4] = r(e, A, s[4]), s[5] = r(t, i, s[5])), ("ending-points" === n.pointsToMove || g) && (s[6] = r(e, A, s[6]), s[7] = r(t, i, s[7])), o[o.length - 1] = [a[0], s, a[2]])
                }
            }
        },
        M = {
            ismousedown: !1,
            prevX: 0,
            prevY: 0,
            mousedown: function (e) {
                var t = e.pageX - ae.offsetLeft,
                    n = e.pageY - ae.offsetTop,
                    A = this;
                A.prevX = t, A.prevY = n, A.ismousedown = !0, h.lineCap = "round", H.pencil(h, [A.prevX, A.prevY, t, n]), o[o.length] = ["pencil", [A.prevX, A.prevY, t, n], H.getOptions(), "start"], A.prevX = t, A.prevY = n
            },
            mouseup: function (e) {
                var t = e.pageX - ae.offsetLeft,
                    n = e.pageY - ae.offsetTop,
                    A = this;
                A.ismousedown && (h.lineCap = "round", H.pencil(h, [A.prevX, A.prevY, t, n]), o[o.length] = ["pencil", [A.prevX, A.prevY, t, n], H.getOptions(), "end"], A.prevX = t, A.prevY = n), this.ismousedown = !1
            },
            mousemove: function (e) {
                var t = e.pageX - ae.offsetLeft,
                    n = e.pageY - ae.offsetTop,
                    A = this;
                A.ismousedown && (h.lineCap = "round", H.pencil(h, [A.prevX, A.prevY, t, n]), o[o.length] = ["pencil", [A.prevX, A.prevY, t, n], H.getOptions()], A.prevX = t, A.prevY = n)
            }
        },
        L = document.getElementById("pencil-stroke-style").value,
        F = "#" + document.getElementById("pencil-fill-style").value,
        H = x(b);
    H.getOptions = function () {
        return [L, F, s, r, l, g, d, c]
    };
    var k = {
            ismousedown: !1,
            prevX: 0,
            prevY: 0,
            mousedown: function (e) {
                var t = e.pageX - ae.offsetLeft,
                    n = e.pageY - ae.offsetTop,
                    A = this;
                A.prevX = t, A.prevY = n, A.ismousedown = !0, h.lineCap = "round", O.line(h, [A.prevX, A.prevY, t, n]), o[o.length] = ["line", [A.prevX, A.prevY, t, n], O.getOptions()], A.prevX = t, A.prevY = n
            },
            mouseup: function (e) {
                this.ismousedown = !1
            },
            mousemove: function (e) {
                var t = e.pageX - ae.offsetLeft,
                    n = e.pageY - ae.offsetTop,
                    A = this;
                A.ismousedown && (h.lineCap = "round", O.line(h, [A.prevX, A.prevY, t, n]), o[o.length] = ["line", [A.prevX, A.prevY, t, n], O.getOptions()], A.prevX = t, A.prevY = n)
            }
        },
        T = document.getElementById("marker-stroke-style").value,
        R = "#" + document.getElementById("marker-fill-style").value,
        O = x(b);
    O.getOptions = function () {
        return [T, R, s, .7, l, g, d, c]
    };
    var S = {
            ismousedown: !1,
            prevX: 0,
            prevY: 0,
            mousedown: function (e) {
                var t = e.pageX - ae.offsetLeft,
                    n = e.pageY - ae.offsetTop,
                    A = this;
                A.prevX = t, A.prevY = n, A.ismousedown = !0, h.lineCap = "round", b.line(h, [A.prevX, A.prevY, t, n]), o[o.length] = ["line", [A.prevX, A.prevY, t, n], b.getOptions()], A.prevX = t, A.prevY = n
            },
            mouseup: function (e) {
                this.ismousedown = !1
            },
            mousemove: function (e) {
                var t = e.pageX - ae.offsetLeft,
                    n = e.pageY - ae.offsetTop,
                    A = this;
                A.ismousedown && (h.lineCap = "round", b.line(h, [A.prevX, A.prevY, t, n]), o[o.length] = ["line", [A.prevX, A.prevY, t, n], b.getOptions()], A.prevX = t, A.prevY = n)
            }
        },
        X = {
            text: "",
            selectedFontFamily: "Arial",
            selectedFontSize: "15",
            lastFillStyle: "",
            onShapeSelected: function () {
                h.canvas.style.cursor = "text", this.x = this.y = this.pageX = this.pageY = 0, this.text = ""
            },
            onShapeUnSelected: function () {
                this.text = "", this.showOrHideTextTools("hide"), h.canvas.style.cursor = "default", void 0 !== this.blinkCursorInterval && clearInterval(this.blinkCursorInterval)
            },
            getFillColor: function (e) {
                return "rgba(255, 255, 255, 0)" == (e = (e || s).toLowerCase()) || "transparent" == e || "white" === e ? "black" : e
            },
            writeText: function (t, n) {
                if (e.isText) {
                    if (n) return X.text = X.text.substr(0, X.text.length - 1), void X.fillText(X.text);
                    X.text += t, X.fillText(X.text)
                }
            },
            fillText: function (t) {
                if (e.isText) {
                    h.clearRect(0, 0, h.canvas.width, h.canvas.height);
                    var n = X.getOptions();
                    b.handleOptions(h, n), h.fillStyle = X.getFillColor(n[2]), h.font = X.selectedFontSize + 'px "' + X.selectedFontFamily + '"', h.fillText(t, X.x, X.y)
                }
            },
            blinkCursorInterval: null,
            index: 0,
            blinkCursor: function () {
                X.index++, X.index % 2 == 0 ? X.fillText(X.text + "|") : X.fillText(X.text)
            },
            getOptions: function () {
                var e = {
                    font: X.selectedFontSize + 'px "' + X.selectedFontFamily + '"',
                    fillStyle: X.getFillColor(),
                    strokeStyle: "#6c96c8",
                    globalCompositeOperation: "source-over",
                    globalAlpha: 1,
                    lineJoin: "round",
                    lineCap: "round",
                    lineWidth: 2
                };
                return c = e.font, e
            },
            appendPoints: function () {
                var e = X.getOptions();
                o[o.length] = ["text", ['"' + X.text + '"', X.x, X.y], b.getOptions(e)]
            },
            mousedown: function (t) {
                e.isText && (X.text.length && this.appendPoints(), X.x = X.y = 0, X.text = "", X.pageX = t.pageX, X.pageY = t.pageY, X.x = t.pageX - ae.offsetLeft - 5, X.y = t.pageY - ae.offsetTop + 10, void 0 !== X.blinkCursorInterval && clearInterval(X.blinkCursorInterval), X.blinkCursor(), X.blinkCursorInterval = setInterval(X.blinkCursor, 700), this.showTextTools())
            },
            mouseup: function (e) {},
            mousemove: function (e) {},
            showOrHideTextTools: function (e) {
                "hide" === e ? this.lastFillStyle.length && (s = this.lastFillStyle, this.lastFillStyle = "") : this.lastFillStyle.length || (this.lastFillStyle = s, s = "black"), this.fontFamilyBox.style.display = "show" == e ? "block" : "none", this.fontSizeBox.style.display = "show" == e ? "block" : "none", this.fontSizeBox.style.left = this.x + "px", this.fontFamilyBox.style.left = this.fontSizeBox.clientWidth + this.x + "px", this.fontSizeBox.style.top = this.y + "px", this.fontFamilyBox.style.top = this.y + "px"
            },
            showTextTools: function () {
                this.fontFamilyBox && this.fontSizeBox && (this.unselectAllFontFamilies(), this.unselectAllFontSizes(), this.showOrHideTextTools("show"), this.eachFontFamily(function (e) {
                    e.onclick = function (e) {
                        e.preventDefault(), X.showOrHideTextTools("hide"), X.selectedFontFamily = this.innerHTML, this.className = "font-family-selected"
                    }, e.style.fontFamily = e.innerHTML
                }), this.eachFontSize(function (e) {
                    e.onclick = function (e) {
                        e.preventDefault(), X.showOrHideTextTools("hide"), X.selectedFontSize = this.innerHTML, this.className = "font-family-selected"
                    }
                }))
            },
            eachFontFamily: function (e) {
                for (var t = this.fontFamilyBox.querySelectorAll("li"), n = 0; n < t.length; n++) e(t[n])
            },
            unselectAllFontFamilies: function () {
                this.eachFontFamily(function (e) {
                    e.className = "", e.innerHTML === X.selectedFontFamily && (e.className = "font-family-selected")
                })
            },
            eachFontSize: function (e) {
                for (var t = this.fontSizeBox.querySelectorAll("li"), n = 0; n < t.length; n++) e(t[n])
            },
            unselectAllFontSizes: function () {
                this.eachFontSize(function (e) {
                    e.className = "", e.innerHTML === X.selectedFontSize && (e.className = "font-size-selected")
                })
            },
            onReturnKeyPressed: function () {
                if (X.text && X.text.length) {
                    var e = parseInt(X.selectedFontSize) || 15;
                    this.mousedown({
                        pageX: this.pageX,
                        pageY: this.pageY + e + 5
                    }), b.redraw()
                }
            },
            fontFamilyBox: document.querySelector(".fontSelectUl"),
            fontSizeBox: document.querySelector(".fontSizeUl")
        },
        K = {
            global: {
                ismousedown: !1,
                prevX: 0,
                prevY: 0,
                prevRadius: 0,
                isCircleDrawn: !1,
                isCircledEnded: !0,
                isClockwise: !1,
                arcRangeContainer: null,
                arcRange: null
            },
            mousedown: function (e) {
                var t = this.global,
                    n = e.pageX - ae.offsetLeft,
                    o = e.pageY - ae.offsetTop;
                t.prevX = n, t.prevY = o, t.ismousedown = !0
            },
            mouseup: function (e) {
                var t = this.global,
                    n = e.pageX - ae.offsetLeft,
                    A = e.pageY - ae.offsetTop;
                if (t.ismousedown)
                    if (!t.isCircleDrawn && t.isCircledEnded) {
                        var i, a = t.prevX,
                            s = t.prevY,
                            r = (n - a + (A - s)) / 3;
                        t.prevRadius = r, t.isCircleDrawn = !0, t.isCircleEnded = !1, i = ((a > n ? a - n : n - a) + (s > A ? s - A : A - s)) / (2 * (2 * Math.PI * r / 21)), o[o.length] = ["arc", [a + r, s + r, r, i, 1], b.getOptions()];
                        var l = t.arcRange,
                            g = t.arcRangeContainer;
                        g.style.display = "block", l.focus(), g.style.top = A + ae.offsetTop + 20 + "px", g.style.left = n + "px", l.value = 2
                    } else t.isCircleDrawn && !t.isCircleEnded && this.end();
                t.ismousedown = !1, this.fixAllPoints()
            },
            mousemove: function (e) {
                var t = this.global,
                    n = e.pageX - ae.offsetLeft,
                    o = e.pageY - ae.offsetTop,
                    A = t.ismousedown,
                    i = t.isCircleDrawn,
                    a = t.isCircledEnded;
                if (A && !i && a) {
                    var s = t.prevX,
                        r = t.prevY,
                        l = (n - s + (o - r)) / 3;
                    h.clearRect(0, 0, 2e3, 2e3), b.arc(h, [s + l, r + l, l, 2 * Math.PI, !0])
                }
            },
            fixAllPoints: function () {
                for (var e = this.toFixed, t = 0; t < o.length; t++) {
                    var n, A = o[t];
                    "arc" === A[0] && (n = A[1], o[t] = ["arc", [e(n[0]), e(n[1]), e(n[2]), e(n[3]), n[4]], A[2]])
                }
            },
            init: function () {
                var e = n("is-clockwise"),
                    A = this.global;
                A.arcRangeContainer = n("arc-range-container"), A.arcRange = n("arc-range"), t(e, "change", function (t) {
                    if (A.isClockwise = e.checked, A.arcRange.value = K.toFixed(A.arcRange.value), A.arcRange.focus(), K.arcRangeHandler(t), o.length) {
                        var n = o[o.length - 1][1];
                        h.clearRect(0, 0, innerWidth, innerHeight), b.arc(h, [n[0], n[1], n[2], n[3], n[4]])
                    }
                });
                var i = A.arcRange;
                t(i, "keydown", this.arcRangeHandler), t(i, "focus", this.arcRangeHandler)
            },
            arcRangeHandler: function (e) {
                var t = K.global,
                    n = t.arcRange,
                    A = e.keyCode,
                    i = +n.value;
                if (39 != A && 40 != A || (n.value = (i < 2 ? i : 1.98) + .02), 37 != A && 38 != A || (n.value = (i > 0 ? i : .02) - .02), !A || 13 == A || 39 == A || 40 == A || 37 == A || 38 == A) {
                    var a = Math.PI * K.toFixed(i),
                        s = o[o.length - 1];
                    if ("arc" === s[0]) {
                        var r = s[1];
                        o[o.length - 1] = ["arc", [r[0], r[1], r[2], a, t.isClockwise ? 1 : 0], s[2]], b.redraw()
                    }
                }
            },
            toFixed: function (e) {
                return Number(e).toFixed(1)
            },
            end: function () {
                var e = this.global;
                e.arcRangeContainer.style.display = "none", e.arcRange.value = 2, e.isCircleDrawn = !1, e.isCircleEnded = !0, b.redraw()
            }
        };
    K.init();
    var U = {
            ismousedown: !1,
            prevX: 0,
            prevY: 0,
            mousedown: function (e) {
                var t = e.pageX - ae.offsetLeft,
                    n = e.pageY - ae.offsetTop,
                    o = this;
                o.prevX = t, o.prevY = n, o.ismousedown = !0
            },
            mouseup: function (e) {
                var t = e.pageX - ae.offsetLeft,
                    n = e.pageY - ae.offsetTop,
                    A = this;
                A.ismousedown && (o[o.length] = ["line", [A.prevX, A.prevY, t, n], b.getOptions()], A.ismousedown = !1)
            },
            mousemove: function (e) {
                var t = e.pageX - ae.offsetLeft,
                    n = e.pageY - ae.offsetTop,
                    o = this;
                o.ismousedown && (h.clearRect(0, 0, innerWidth, innerHeight), b.line(h, [o.prevX, o.prevY, t, n]))
            }
        },
        G = {
            ismousedown: !1,
            prevX: 0,
            prevY: 0,
            arrowSize: 10,
            mousedown: function (e) {
                var t = e.pageX - ae.offsetLeft,
                    n = e.pageY - ae.offsetTop,
                    o = this;
                o.prevX = t, o.prevY = n, o.ismousedown = !0
            },
            mouseup: function (e) {
                var t = e.pageX - ae.offsetLeft,
                    n = e.pageY - ae.offsetTop,
                    A = this;
                A.ismousedown && (o[o.length] = ["arrow", [A.prevX, A.prevY, t, n], b.getOptions()], A.ismousedown = !1)
            },
            mousemove: function (e) {
                var t = e.pageX - ae.offsetLeft,
                    n = e.pageY - ae.offsetTop,
                    o = this;
                o.ismousedown && (h.clearRect(0, 0, innerWidth, innerHeight), b.arrow(h, [o.prevX, o.prevY, t, n]))
            }
        },
        z = {
            ismousedown: !1,
            prevX: 0,
            prevY: 0,
            mousedown: function (e) {
                var t = e.pageX - ae.offsetLeft,
                    n = e.pageY - ae.offsetTop,
                    o = this;
                o.prevX = t, o.prevY = n, o.ismousedown = !0
            },
            mouseup: function (e) {
                var t = e.pageX - ae.offsetLeft,
                    n = e.pageY - ae.offsetTop,
                    A = this;
                A.ismousedown && (o[o.length] = ["rect", [A.prevX, A.prevY, t - A.prevX, n - A.prevY], b.getOptions()], A.ismousedown = !1)
            },
            mousemove: function (e) {
                var t = e.pageX - ae.offsetLeft,
                    n = e.pageY - ae.offsetTop,
                    o = this;
                o.ismousedown && (h.clearRect(0, 0, innerWidth, innerHeight), b.rect(h, [o.prevX, o.prevY, t - o.prevX, n - o.prevY]))
            }
        },
        J = {
            global: {
                ismousedown: !1,
                prevX: 0,
                prevY: 0,
                controlPointX: 0,
                controlPointY: 0,
                isFirstStep: !0,
                isLastStep: !1
            },
            mousedown: function (e) {
                var t = this.global,
                    n = e.pageX - ae.offsetLeft,
                    o = e.pageY - ae.offsetTop;
                t.isLastStep || (t.prevX = n, t.prevY = o), t.ismousedown = !0, t.isLastStep && t.ismousedown && this.end(n, o)
            },
            mouseup: function (e) {
                var t = this.global,
                    n = e.pageX - ae.offsetLeft,
                    o = e.pageY - ae.offsetTop;
                t.ismousedown && t.isFirstStep && (t.controlPointX = n, t.controlPointY = o, t.isFirstStep = !1, t.isLastStep = !0)
            },
            mousemove: function (e) {
                var t = e.pageX - ae.offsetLeft,
                    n = e.pageY - ae.offsetTop,
                    o = this.global;
                h.clearRect(0, 0, innerWidth, innerHeight), o.ismousedown && o.isFirstStep && b.quadratic(h, [o.prevX, o.prevY, t, n, t, n]), o.isLastStep && b.quadratic(h, [o.prevX, o.prevY, o.controlPointX, o.controlPointY, t, n])
            },
            end: function (e, t) {
                var n = this.global;
                n.ismousedown && (n.isLastStep = !1, n.isFirstStep = !0, n.ismousedown = !1, e = e || n.controlPointX || n.prevX, t = t || n.controlPointY || n.prevY, o[o.length] = ["quadratic", [n.prevX, n.prevY, n.controlPointX, n.controlPointY, e, t], b.getOptions()])
            }
        },
        N = {
            global: {
                ismousedown: !1,
                prevX: 0,
                prevY: 0,
                firstControlPointX: 0,
                firstControlPointY: 0,
                secondControlPointX: 0,
                secondControlPointY: 0,
                isFirstStep: !0,
                isSecondStep: !1,
                isLastStep: !1
            },
            mousedown: function (e) {
                var t = this.global,
                    n = e.pageX - ae.offsetLeft,
                    o = e.pageY - ae.offsetTop;
                t.isLastStep || t.isSecondStep || (t.prevX = n, t.prevY = o), t.ismousedown = !0, t.isLastStep && t.ismousedown && this.end(n, o), t.ismousedown && t.isSecondStep && (t.secondControlPointX = n, t.secondControlPointY = o, t.isSecondStep = !1, t.isLastStep = !0)
            },
            mouseup: function (e) {
                var t = this.global,
                    n = e.pageX - ae.offsetLeft,
                    o = e.pageY - ae.offsetTop;
                t.ismousedown && t.isFirstStep && (t.firstControlPointX = n, t.firstControlPointY = o, t.isFirstStep = !1, t.isSecondStep = !0)
            },
            mousemove: function (e) {
                var t = e.pageX - ae.offsetLeft,
                    n = e.pageY - ae.offsetTop,
                    o = this.global;
                h.clearRect(0, 0, innerWidth, innerHeight), o.ismousedown && o.isFirstStep && b.bezier(h, [o.prevX, o.prevY, t, n, t, n, t, n]), o.ismousedown && o.isSecondStep && b.bezier(h, [o.prevX, o.prevY, o.firstControlPointX, o.firstControlPointY, t, n, t, n]), o.isLastStep && b.bezier(h, [o.prevX, o.prevY, o.firstControlPointX, o.firstControlPointY, o.secondControlPointX, o.secondControlPointY, t, n])
            },
            end: function (e, t) {
                var n = this.global;
                n.ismousedown && (n.isLastStep = n.isSecondStep = !1, n.isFirstStep = !0, n.ismousedown = !1, n.secondControlPointX = n.secondControlPointX || n.firstControlPointX, n.secondControlPointY = n.secondControlPointY || n.firstControlPointY, e = e || n.secondControlPointX, t = t || n.secondControlPointY, o[o.length] = ["bezier", [n.prevX, n.prevY, n.firstControlPointX, n.firstControlPointY, n.secondControlPointX, n.secondControlPointY, e, t], b.getOptions()])
            }
        },
        j = {
            scale: 1,
            up: function (e) {
                this.scale += .01, this.apply()
            },
            down: function (e) {
                this.scale -= .01, this.apply()
            },
            apply: function () {
                h.scale(this.scale, this.scale), p.scale(this.scale, this.scale), b.redraw()
            },
            icons: {
                up: function (e) {
                    e.font = "22px Verdana", e.strokeText("+", 10, 30)
                },
                down: function (e) {
                    e.font = "22px Verdana", e.strokeText("-", 15, 30)
                }
            }
        },
        V = function () {
            function e(e, t, n) {
                var o, A, i = document.createElement("input");
                i.type = "file", t && (i.multiple = !0), i.accept = n || "image/*", i.onchange = function () {
                    if (t) return i.files.length ? void e(i.files) : void console.error("No file selected.");
                    i.files[0] ? (e(i.files[0]), i.parentNode.removeChild(i)) : console.error("No file selected.")
                }, i.style.display = "none", (document.body || document.documentElement).appendChild(i), o = i, A = new window.MouseEvent("click", {
                    view: window,
                    bubbles: !0,
                    cancelable: !0,
                    button: 0,
                    buttons: 0,
                    mozInputSource: 1
                }), o.dispatchEvent(A)
            }
            this.selectSingleFile = e, this.selectMultipleFiles = function (t) {
                e(t, !0)
            }
        },
        W = {
            lastImageURL: null,
            lastImageIndex: 0,
            images: [],
            ismousedown: !1,
            prevX: 0,
            prevY: 0,
            load: function (e, t) {
                var n = W;
                o[o.length] = ["image", [W.lastImageURL, n.prevX, n.prevY, e, t, W.lastImageIndex], b.getOptions()], document.getElementById("drag-last-path").click(), de(!0)
            },
            mousedown: function (e) {
                var t = e.pageX - ae.offsetLeft,
                    n = e.pageY - ae.offsetTop,
                    o = this;
                o.prevX = t, o.prevY = n, o.ismousedown = !0
            },
            mouseup: function (e) {
                var t = e.pageX - ae.offsetLeft,
                    n = e.pageY - ae.offsetTop,
                    A = this;
                A.ismousedown && (o[o.length] = ["image", [W.lastImageURL, A.prevX, A.prevY, t - A.prevX, n - A.prevY, W.lastImageIndex], b.getOptions()], A.ismousedown = !1)
            },
            mousemove: function (e) {
                var t = e.pageX - ae.offsetLeft,
                    n = e.pageY - ae.offsetTop,
                    o = this;
                o.ismousedown && (h.clearRect(0, 0, innerWidth, innerHeight), b.image(h, [W.lastImageURL, o.prevX, o.prevY, t - o.prevX, n - o.prevY, W.lastImageIndex]))
            }
        },
        Z = {
            lastPdfURL: null,
            lastIndex: 0,
            lastPointIndex: 0,
            removeWhiteBackground: !1,
            pdfPageContainer: document.getElementById("pdf-page-container"),
            pdfPagesList: document.getElementById("pdf-pages-list"),
            pdfNext: document.getElementById("pdf-next"),
            pdfPrev: document.getElementById("pdf-prev"),
            pdfClose: document.getElementById("pdf-close"),
            pageNumber: 1,
            images: [],
            ismousedown: !1,
            prevX: 0,
            prevY: 0,
            getPage: function (e, t) {
                if (e = parseInt(e) || 1, !Z.pdf) return pdfjsLib.disableWorker = !1, void pdfjsLib.getDocument(Z.lastPdfURL).then(function (n) {
                    Z.pdf = n, Z.getPage(e, t)
                });
                var n = Z.pdf;
                n.getPage(e).then(function (o) {
                    Z.pageNumber = e;
                    var A = o.getViewport(1.5),
                        i = document.createElement("canvas"),
                        a = i.getContext("2d");
                    i.height = A.height, i.width = A.width;
                    var s = {
                        canvasContext: a,
                        viewport: A
                    };
                    !0 === Z.removeWhiteBackground && (s.background = "rgba(0,0,0,0)"), o.render(s).then(function () {
                        if (!0 === Z.removeWhiteBackground) {
                            for (var e = a.getImageData(0, 0, i.width, i.height), o = e.data, A = {
                                    r: 0,
                                    g: 0,
                                    b: 0,
                                    a: 0
                                }, s = 0, r = o.length; s < r; s += 4) {
                                var l = o[s],
                                    g = o[s + 1],
                                    c = o[s + 2];
                                255 == l && 255 == g && 255 == c && (o[s] = A.r, o[s + 1] = A.g, o[s + 2] = A.b, o[s + 3] = A.a)
                            }
                            a.putImageData(e, 0, 0)
                        }
                        Z.lastPage = i.toDataURL("image/png"), t(Z.lastPage, i.width, i.height, n.numPages)
                    })
                })
            },
            load: function (e) {
                Z.lastPdfURL = e, Z.getPage(parseInt(Z.pdfPagesList.value || 1), function (t, n, A, i) {
                    Z.prevX = ae.width - n - parseInt(n / 2);
                    Z.lastIndex = Z.images.length;
                    var a = [t, 60, 20, n, A, Z.lastIndex];
                    Z.lastPointIndex = o.length, o[o.length] = ["pdf", a, b.getOptions()], Z.pdfPagesList.innerHTML = "";
                    for (var s = 1; s <= i; s++) {
                        var r = document.createElement("option");
                        r.value = s, r.innerHTML = "Page #" + s, Z.pdfPagesList.appendChild(r), Z.pageNumber.toString() == s.toString() && (r.selected = !0)
                    }
                    Z.pdfPagesList.onchange = function () {
                        Z.load(e)
                    }, Z.pdfNext.onclick = function () {
                        Z.pdfPagesList.selectedIndex++, Z.pdfPagesList.onchange()
                    }, Z.pdfPrev.onclick = function () {
                        Z.pdfPagesList.selectedIndex--, Z.pdfPagesList.onchange()
                    }, Z.pdfClose.onclick = function () {
                        Z.pdfPageContainer.style.display = "none"
                    }, document.getElementById("drag-last-path").click(), Z.pdfPrev.src = _.pdf_next, Z.pdfNext.src = _.pdf_prev, Z.pdfClose.src = _.pdf_close, Z.pdfPageContainer.style.top = "20px", Z.pdfPageContainer.style.left = a[3] - parseInt(a[3] / 2) + "px", Z.pdfPageContainer.style.display = "block", de(!0)
                })
            },
            mousedown: function (e) {
                var t = e.pageX - ae.offsetLeft,
                    n = e.pageY - ae.offsetTop,
                    o = this;
                o.prevX = t, o.prevY = n, o.ismousedown = !0
            },
            mouseup: function (e) {
                var t = e.pageX - ae.offsetLeft,
                    n = e.pageY - ae.offsetTop,
                    A = this;
                A.ismousedown && (o[Z.lastPointIndex] && (o[Z.lastPointIndex] = ["pdf", [Z.lastPage, A.prevX, A.prevY, t - A.prevX, n - A.prevY, Z.lastIndex], b.getOptions()]), A.ismousedown = !1)
            },
            mousemove: function (e) {
                var t = e.pageX - ae.offsetLeft,
                    n = e.pageY - ae.offsetTop,
                    o = this;
                o.ismousedown && (h.clearRect(0, 0, innerWidth, innerHeight), b.pdf(h, [Z.lastPage, o.prevX, o.prevY, t - o.prevX, n - o.prevY, Z.lastIndex]))
            },
            reset_pos: function (e, t) {
                if (Z.pdfPageContainer.style.top = t + "px", o[Z.lastPointIndex]) {
                    var n = o[Z.lastPointIndex][1];
                    Z.pdfPageContainer.style.left = n[1] + n[3] - parseInt(n[3] / 2) - parseInt(Z.pdfPageContainer.clientWidth / 2) + "px"
                }
            },
            end: function () {}
        },
        q = {};
    if (params.icons) try {
        q = JSON.parse(params.icons)
    } catch (e) {
        q = {}
    }
    var _ = {
            line: q.line || "data:image/png;base64,",
            arrow: q.arrow || "data:image/png;base64,",
            pencil: q.pencil || "data:image/png;base64,",
            marker: q.marker || "data:image/png;base64,",
            dragSingle: q.dragSingle || "data:image/png;base64,",
            dragMultiple: q.dragMultiple || "data:image/png;base64,",
            eraser: q.eraser || "data:image/png;base64,",
            rectangle: q.rectangle || "data:image/png;base64,",
            arc: q.arc || "data:image/png;base64,",
            bezier: q.bezier || "data:image/png;base64,",
            quadratic: q.quadratic || "data:image/png;base64,",
            text: q.text || "data:image/png;base64,",
            image: q.image || "data:image/png;base64,",
            pdf: q.pdf || "data:image/png;base64,",
            zoom_in: q.zoom_in || "data:image/png;base64,",
            zoom_out: q.zoom_out || "data:image/png;base64,",
            lineWidth: q.lineWidth || "data:image/png;base64,",
            colorsPicker: q.colorsPicker || "data:image/png;base64,",
            extraOptions: q.extraOptions || "data:image/png;base64,",
            undo: q.undo || "data:image/png;base64,",
            pdf_next: q.pdf_next || "data:image/png;base64,",
            pdf_prev: q.pdf_prev || "data:image/png;base64,",
            pdf_close: q.pdf_close || "data:image/png;base64,",
            snap: q.snap,
            clear: q.clear
        },
        $ = {
            line: !0,
            arrow: !0,
            pencil: !0,
            marker: !0,
            dragSingle: !0,
            dragMultiple: !0,
            eraser: !0,
            rectangle: !0,
            arc: !0,
            bezier: !0,
            quadratic: !0,
            text: !0,
            image: !0,
            pdf: !0,
            zoom: !0,
            lineWidth: !0,
            colorsPicker: !0,
            extraOptions: !0,
            code: !0,
            undo: !0,
            snap: !0
        };
    if (params.tools) try {
        var ee = JSON.parse(params.tools);
        $ = ee
    } catch (e) {}

    function te(t, n) {
        v(), oe(), e.set(n);
        var o = document.getElementsByClassName("selected-shape")[0];
        o && (o.className = o.className.replace(/selected-shape/g, "")), t.className || (t.className = ""), t.className += " selected-shape"
    }

    function ne() {
        for (var e, t = document.getElementById("tool-box").getElementsByTagName("canvas"), n = window.selectedIcon.toLowerCase(), o = 0; o < t.length; o++) e || -1 === (t[o].id || "").indexOf(n) || (e = t[o]);
        e || (window.selectedIcon = "Pencil", e = document.getElementById("pencil-icon")), te(e, window.selectedIcon)
    }

    function oe() {
        var e = n("additional-container"),
            t = n("colors-container"),
            o = n("marker-container"),
            A = n("marker-fill-colors"),
            i = n("pencil-container"),
            a = n("pencil-fill-colors"),
            s = n("line-width-container");
        e.style.display = t.style.display = A.style.display = o.style.display = a.style.display = i.style.display = s.style.display = "none"
    }

    function Ae() {
        o.push(["line", [139, 261, 170, 219],
            [1, "rgba(0,0,0,0)", "rgba(0,0,0,0)", 1, "source-over", "round", "round", '15px "Arial"']
        ]), b.redraw(), setTimeout(function () {
            te(document.getElementById("line"), "Line")
        }, 1e3), setTimeout(ne, 2e3)
    }!0 === $.code && (document.querySelector(".preview-panel").style.display = "block"), e.set(window.selectedIcon), window.addEventListener("load", function () {
            ne()
        }, !1),
        function () {
            var e, A, c = {},
                f = n("lineCap-select"),
                p = n("lineJoin-select");

            function h(e) {
                var t = n(e).getContext("2d");
                return t.lineWidth = 2, t.strokeStyle = "#6c96c8", t
            }

            function w(e, o) {
                "Pencil" !== o && "Marker" !== o || (g = d = "round"), t(e.canvas, "click", function () {
                    var e;
                    (X.text.length && X.appendPoints(), "Text" === o ? X.onShapeSelected() : X.onShapeUnSelected(), "Pencil" !== o && "Marker" !== o || (g = d = "round"), y.global.startingIndex = 0, te(this, o), "drag-last-path" === this.id ? (n("copy-last").checked = !0, n("copy-all").checked = !1) : "drag-all-paths" === this.id && (n("copy-all").checked = !0, n("copy-last").checked = !1), "image-icon" === this.id) && ((e = new V).accept = "image/*", e.selectSingleFile(function (e) {
                        if (e) {
                            var t = new FileReader;
                            t.onload = function (e) {
                                var t = new Image;
                                t.onload = function () {
                                    var e = W.images.length;
                                    W.lastImageURL = t.src, W.lastImageIndex = e, W.images.push(t), W.load(t.clientWidth, t.clientHeight)
                                }, t.style = "position: absolute; top: -99999999999; left: -999999999;", document.body.appendChild(t), t.src = e.target.result
                            }, t.readAsDataURL(e)
                        }
                    }));
                    "pdf-icon" === this.id && (e = new V).selectSingleFile(function (e) {
                        var t;
                        e && ((t = new FileReader).onload = function (e) {
                            Z.pdf = null, Z.load(e.target.result)
                        }, t.readAsDataURL(e))
                    }, null, "application/pdf");
                    "pencil-icon" === this.id || "eraser-icon" === this.id || "marker-icon" === this.id ? (c.lineCap = g, c.lineJoin = d, g = d = "round") : c.lineCap && c.lineJoin && (g = c.lineCap, d = c.lineJoin), "eraser-icon" === this.id ? (c.strokeStyle = a, c.fillStyle = s, c.lineWidth = i, a = "White", s = "White", i = 10) : c.strokeStyle && c.fillStyle && void 0 !== c.lineWidth && (a = c.strokeStyle, s = c.fillStyle, i = c.lineWidth)
                })
            }
            n("tool-box").style.height = innerHeight + "px", e = h("drag-last-path"), (A = new Image).onload = function () {
                    e.drawImage(A, 4, 4, 32, 32), w(e, "DragLastPath")
                }, A.src = _.dragSingle, !0 === $.dragSingle && (document.getElementById("drag-last-path").style.display = "block"),
                function () {
                    var e = h("drag-all-paths"),
                        t = new Image;
                    t.onload = function () {
                        e.drawImage(t, 4, 4, 32, 32), w(e, "DragAllPaths")
                    }, t.src = _.dragMultiple
                }(), !0 === $.dragMultiple && (document.getElementById("drag-all-paths").style.display = "block"), !0 === $.line && (! function () {
                    var e = h("line"),
                        t = new Image;
                    t.onload = function () {
                        e.drawImage(t, 4, 4, 32, 32), w(e, "Line")
                    }, t.src = _.line
                }(), document.getElementById("line").style.display = "block"), !0 === $.snap && (! function () {
                    var e = h("snap"),
                        t = new Image;
                    t.onload = function () {
                        e.drawImage(t, 4, 4, 32, 32), document.querySelector("#snap").onclick = function () {
                            let e = document.createElement("a");
                            e.setAttribute("download", (new Date).getTime() + ".png");
                            let t = document.getElementById("main-canvas").toDataURL("image/png").replace(/^data:image\/png/, "data:application/octet-stream");
                            e.setAttribute("href", t), e.click()
                        }
                    }, t.src = _.snap
                }(), document.getElementById("snap").style.display = "block"), !0 === $.clear && (! function () {
                    var e = h("clear"),
                        t = new Image;
                    t.onload = function () {
                        e.drawImage(t, 4, 4, 32, 32), document.querySelector("#clear").onclick = function () {
                            o = [], b.redraw(), de(!0)
                        }
                    }, t.src = _.clear
                }(), document.getElementById("clear").style.display = "block"), !0 === $.undo && (! function () {
                    var e = h("undo"),
                        t = new Image;
                    t.onload = function () {
                        e.drawImage(t, 4, 4, 32, 32), document.querySelector("#undo").onclick = function () {
                            o.length && (o.length = o.length - 1, b.redraw()), de(!0)
                        }
                    }, t.src = _.undo
                }(), document.getElementById("undo").style.display = "block"), !0 === $.arrow && (! function () {
                    var e = h("arrow"),
                        t = new Image;
                    t.onload = function () {
                        e.drawImage(t, 4, 4, 32, 32), w(e, "Arrow")
                    }, t.src = _.arrow
                }(), document.getElementById("arrow").style.display = "block"), !0 === $.zoom && (! function () {
                    var e = h("zoom-up");
                    t(e.canvas, "click", function () {
                        j.up()
                    });
                    var n = new Image;
                    n.onload = function () {
                        e.drawImage(n, 4, 4, 32, 32)
                    }, n.src = _.zoom_in
                }(), function () {
                    var e = h("zoom-down");
                    t(e.canvas, "click", function () {
                        j.down()
                    });
                    var n = new Image;
                    n.onload = function () {
                        e.drawImage(n, 4, 4, 32, 32)
                    }, n.src = _.zoom_out
                }(), document.getElementById("zoom-up").style.display = "block", document.getElementById("zoom-down").style.display = "block"), !0 === $.pencil && (! function () {
                    function e(e, t) {
                        return "rgba(" + Y(e).join(",") + ",1)"
                    }
                    var o = h("pencil-icon"),
                        A = new Image;
                    A.onload = function () {
                        o.drawImage(A, 4, 4, 32, 32), w(o, "Pencil")
                    }, A.src = _.pencil;
                    var i = n("pencil-container"),
                        a = n("pencil-fill-colors"),
                        s = n("pencil-stroke-style"),
                        r = n("pencil-colors-list"),
                        l = n("pencil-fill-style"),
                        g = n("pencil-selected-color"),
                        c = n("pencil-selected-color-2"),
                        d = n("pencil-done"),
                        f = o.canvas;
                    F = e(l.value), g.style.backgroundColor = c.style.backgroundColor = "#" + l.value, [
                        ["FFFFFF", "006600", "000099", "CC0000", "8C4600"],
                        ["CCCCCC", "00CC00", "6633CC", "FF0000", "B28500"],
                        ["666666", "66FFB2", "006DD9", "FF7373", "FF9933"],
                        ["333333", "26FF26", "6699FF", "CC33FF", "FFCC99"],
                        ["000000", "CCFF99", "BFDFFF", "FFBFBF", "FFFF33"]
                    ].forEach(function (e) {
                        var t = "<tr>";
                        e.forEach(function (e) {
                            t += '<td style="background-color:#' + e + '" data-color="' + e + '"></td>'
                        }), t += "</tr>", r.innerHTML += t
                    }), Array.prototype.slice.call(r.getElementsByTagName("td")).forEach(function (e) {
                        t(e, "mouseover", function () {
                            var t = e.getAttribute("data-color");
                            c.style.backgroundColor = "#" + t, l.value = t
                        }), t(e, "click", function () {
                            var t = e.getAttribute("data-color");
                            g.style.backgroundColor = c.style.backgroundColor = "#" + t, l.value = t, a.style.display = "none"
                        })
                    }), t(f, "click", function () {
                        oe(), i.style.display = "block", i.style.top = f.offsetTop + 1 + "px", i.style.left = f.offsetLeft + f.clientWidth + "px", l.focus()
                    }), t(d, "click", function () {
                        i.style.display = "none", a.style.display = "none", L = s.value, F = e(l.value)
                    }), t(g, "click", function () {
                        a.style.display = "block"
                    })
                }(), document.getElementById("pencil-icon").style.display = "block"), !0 === $.marker && (! function () {
                    function e(e, t) {
                        return "rgba(" + Y(e).join(",") + "," + t + ")"
                    }
                    var o = h("marker-icon"),
                        A = new Image;
                    A.onload = function () {
                        o.drawImage(A, 4, 4, 32, 32), w(o, "Marker")
                    }, A.src = _.marker;
                    var i = n("marker-container"),
                        a = n("marker-fill-colors"),
                        s = n("marker-stroke-style"),
                        r = n("marker-colors-list"),
                        l = n("marker-fill-style"),
                        g = n("marker-selected-color"),
                        c = n("marker-selected-color-2"),
                        d = n("marker-done"),
                        f = o.canvas;
                    R = e(l.value, .2), g.style.backgroundColor = c.style.backgroundColor = "#" + l.value, [
                        ["FFFFFF", "006600", "000099", "CC0000", "8C4600"],
                        ["CCCCCC", "00CC00", "6633CC", "FF0000", "B28500"],
                        ["666666", "66FFB2", "006DD9", "FF7373", "FF9933"],
                        ["333333", "26FF26", "6699FF", "CC33FF", "FFCC99"],
                        ["000000", "CCFF99", "BFDFFF", "FFBFBF", "FFFF33"]
                    ].forEach(function (e) {
                        var t = "<tr>";
                        e.forEach(function (e) {
                            t += '<td style="background-color:#' + e + '" data-color="' + e + '"></td>'
                        }), t += "</tr>", r.innerHTML += t
                    }), Array.prototype.slice.call(r.getElementsByTagName("td")).forEach(function (e) {
                        t(e, "mouseover", function () {
                            var t = e.getAttribute("data-color");
                            c.style.backgroundColor = "#" + t, l.value = t
                        }), t(e, "click", function () {
                            var t = e.getAttribute("data-color");
                            g.style.backgroundColor = c.style.backgroundColor = "#" + t, l.value = t, a.style.display = "none"
                        })
                    }), t(f, "click", function () {
                        oe(), i.style.display = "block", i.style.top = f.offsetTop + 1 + "px", i.style.left = f.offsetLeft + f.clientWidth + "px", l.focus()
                    }), t(d, "click", function () {
                        i.style.display = "none", a.style.display = "none", T = s.value, R = e(l.value, .2)
                    }), t(g, "click", function () {
                        a.style.display = "block"
                    })
                }(), document.getElementById("marker-icon").style.display = "block"), !0 === $.eraser && (! function () {
                    var e = h("eraser-icon"),
                        t = new Image;
                    t.onload = function () {
                        e.drawImage(t, 4, 4, 32, 32), w(e, "Eraser")
                    }, t.src = _.eraser
                }(), document.getElementById("eraser-icon").style.display = "block"), !0 === $.text && (! function () {
                    var e = h("text-icon"),
                        t = new Image;
                    t.onload = function () {
                        e.drawImage(t, 4, 4, 32, 32), w(e, "Text")
                    }, t.src = _.text
                }(), document.getElementById("text-icon").style.display = "block"), !0 === $.image && (! function () {
                    var e = h("image-icon"),
                        t = new Image;
                    t.onload = function () {
                        e.drawImage(t, 4, 4, 32, 32), w(e, "Image")
                    }, t.src = _.image
                }(), document.getElementById("image-icon").style.display = "block"), !0 === $.pdf && (! function () {
                    var e = h("pdf-icon"),
                        t = new Image;
                    t.onload = function () {
                        e.drawImage(t, 4, 4, 32, 32), w(e, "Pdf")
                    }, t.src = _.pdf
                }(), document.getElementById("pdf-icon").style.display = "block"), !0 === $.arc && (! function () {
                    var e = h("arc"),
                        t = new Image;
                    t.onload = function () {
                        e.drawImage(t, 4, 4, 32, 32), w(e, "Arc")
                    }, t.src = _.arc
                }(), document.getElementById("arc").style.display = "block"), !0 === $.rectangle && (! function () {
                    var e = h("rectangle"),
                        t = new Image;
                    t.onload = function () {
                        e.drawImage(t, 4, 4, 32, 32), w(e, "Rectangle")
                    }, t.src = _.rectangle
                }(), document.getElementById("rectangle").style.display = "block"), !0 === $.quadratic && (! function () {
                    var e = h("quadratic-curve"),
                        t = new Image;
                    t.onload = function () {
                        e.drawImage(t, 4, 4, 32, 32), w(e, "QuadraticCurve")
                    }, t.src = _.quadratic
                }(), document.getElementById("quadratic-curve").style.display = "block"), !0 === $.bezier && (! function () {
                    var e = h("bezier-curve"),
                        t = new Image;
                    t.onload = function () {
                        e.drawImage(t, 4, 4, 32, 32), w(e, "Bezier")
                    }, t.src = _.bezier
                }(), document.getElementById("bezier-curve").style.display = "block"), !0 === $.lineWidth && (! function () {
                    var e = h("line-width"),
                        o = new Image;
                    o.onload = function () {
                        e.drawImage(o, 4, 4, 32, 32)
                    }, o.src = _.lineWidth;
                    var A = n("line-width-container"),
                        a = n("line-width-text"),
                        s = n("line-width-done"),
                        r = (document.getElementsByTagName("h1")[0], e.canvas);
                    t(r, "click", function () {
                        oe(), A.style.display = "block", A.style.top = r.offsetTop + 1 + "px", A.style.left = r.offsetLeft + r.clientWidth + "px", a.focus()
                    }), t(s, "click", function () {
                        A.style.display = "none", i = a.value
                    })
                }(), document.getElementById("line-width").style.display = "block"), !0 === $.colorsPicker && (! function () {
                    var e = h("colors"),
                        o = new Image;
                    o.onload = function () {
                        e.drawImage(o, 4, 4, 32, 32)
                    }, o.src = _.colorsPicker;
                    var A = n("colors-container"),
                        i = n("stroke-style"),
                        r = n("fill-style"),
                        l = n("colors-done"),
                        g = (document.getElementsByTagName("h1")[0], e.canvas);
                    t(g, "click", function () {
                        oe(), A.style.display = "block", A.style.top = g.offsetTop + 1 + "px", A.style.left = g.offsetLeft + g.clientWidth + "px", i.focus()
                    }), t(l, "click", function () {
                        A.style.display = "none", a = i.value, s = r.value
                    })
                }(), document.getElementById("colors").style.display = "block"), !0 === $.extraOptions && (! function () {
                    var e = h("additional"),
                        o = new Image;
                    o.onload = function () {
                        e.drawImage(o, 4, 4, 32, 32)
                    }, o.src = _.extraOptions;
                    var A = n("additional-container"),
                        i = n("additional-close"),
                        a = (document.getElementsByTagName("h1")[0], e.canvas),
                        s = n("globalAlpha-select"),
                        c = n("globalCompositeOperation-select");
                    t(a, "click", function () {
                        oe(), A.style.display = "block", A.style.top = a.offsetTop + 1 + "px", A.style.left = a.offsetLeft + a.clientWidth + "px"
                    }), t(i, "click", function () {
                        A.style.display = "none", r = s.value, l = c.value, g = f.value, d = p.value
                    })
                }(), document.getElementById("additional").style.display = "block");
            var C = n("design-preview"),
                B = n("code-preview");

            function I() {
                m.parentNode.style.display = "none", E.style.display = "none", oe(), v()
            }

            function P() {
                m.parentNode.style.display = "block", E.style.display = "block", m.focus(), u.updateTextArea(), m.style.width = innerWidth - E.clientWidth - 30 + "px", m.style.height = innerHeight - 40 + "px", m.style.marginLeft = E.clientWidth + "px", E.style.height = innerHeight + "px", oe(), v()
            }
            window.selectBtn = function (e, t) {
                B.className = C.className = "", e == C ? C.className = "preview-selected" : B.className = "preview-selected", !t && window.connection && connection.numberOfConnectedUsers >= 1 ? connection.send({
                    btnSelected: e.id
                }) : e == C ? I() : P()
            }, t(C, "click", function () {
                selectBtn(C), I()
            }), t(B, "click", function () {
                selectBtn(B), P()
            });
            var m = n("code-text"),
                E = n("options-container");
            var D = n("is-absolute-points");
            t(n("is-shorten-code"), "change", u.updateTextArea), t(D, "change", u.updateTextArea)
        }();
    var ie, ae = h.canvas,
        se = "createTouch" in document;

    function re(e) {
        e && ("function" == typeof e.preventDefault && e.preventDefault(), "function" == typeof e.stopPropagation && e.stopPropagation())
    }

    function le(e, t) {
        var n = !1,
            o = e.srcElement || e.target;
        return (n = ("INPUT" !== o.tagName.toUpperCase() || "TEXT" !== o.type.toUpperCase() && "PASSWORD" !== o.type.toUpperCase() && "FILE" !== o.type.toUpperCase() && "SEARCH" !== o.type.toUpperCase() && "EMAIL" !== o.type.toUpperCase() && "NUMBER" !== o.type.toUpperCase() && "DATE" !== o.type.toUpperCase()) && "TEXTAREA" !== o.tagName.toUpperCase() || (o.readOnly || o.disabled)) && e.preventDefault(), n
    }
    t(ae, se ? "touchstart mousedown" : "mousedown", function (t) {
        se && (t = t.pageX ? t : t.touches.length ? t.touches[0] : {
            pageX: 0,
            pageY: 0
        });
        var n = e;
        n.isLine ? U.mousedown(t) : n.isArc ? K.mousedown(t) : n.isRectangle ? z.mousedown(t) : n.isQuadraticCurve ? J.mousedown(t) : n.isBezierCurve ? N.mousedown(t) : n.isDragLastPath || n.isDragAllPaths ? y.mousedown(t) : n.isPencil ? M.mousedown(t) : n.isEraser ? S.mousedown(t) : n.isText ? X.mousedown(t) : n.isImage ? W.mousedown(t) : n.isPdf ? Z.mousedown(t) : n.isArrow ? G.mousedown(t) : n.isMarker && k.mousedown(t), !n.isPdf && b.redraw(), re(t)
    }), t(ae, se ? "touchend touchcancel mouseup" : "mouseup", function (t) {
        !se || t && "pageX" in t || (t = t && t.touches && t.touches.length ? t.touches[0] : t && t.changedTouches && t.changedTouches.length ? t.changedTouches[0] : {
            pageX: 0,
            pageY: 0
        });
        var n = e;
        n.isLine ? U.mouseup(t) : n.isArc ? K.mouseup(t) : n.isRectangle ? z.mouseup(t) : n.isQuadraticCurve ? J.mouseup(t) : n.isBezierCurve ? N.mouseup(t) : n.isDragLastPath || n.isDragAllPaths ? y.mouseup(t) : n.isPencil ? M.mouseup(t) : n.isEraser ? S.mouseup(t) : n.isText ? X.mouseup(t) : n.isImage ? W.mouseup(t) : n.isPdf ? Z.mousedown(t) : n.isArrow ? G.mouseup(t) : n.isMarker && k.mouseup(t), !n.isPdf && b.redraw(), de(!(!e.isDragAllPaths && !e.isDragLastPath)), re(t)
    }), t(ae, se ? "touchmove mousemove" : "mousemove", function (t) {
        se && (t = t.pageX ? t : t.touches.length ? t.touches[0] : {
            pageX: 0,
            pageY: 0
        });
        var n = e;
        n.isLine ? U.mousemove(t) : n.isArc ? K.mousemove(t) : n.isRectangle ? z.mousemove(t) : n.isQuadraticCurve ? J.mousemove(t) : n.isBezierCurve ? N.mousemove(t) : n.isDragLastPath || n.isDragAllPaths ? y.mousemove(t) : n.isPencil ? M.mousemove(t) : n.isEraser ? S.mousemove(t) : n.isText ? X.mousemove(t) : n.isImage ? W.mousemove(t) : n.isPdf ? Z.mousedown(t) : n.isArrow ? G.mousemove(t) : n.isMarker && k.mousemove(t), re(t)
    }), t(document, "keydown", function (e) {
        8 != (ie = e.which || e.keyCode || 0) && 46 != ie ? (e.metaKey && (C = !0, ie = 17), C || 17 !== ie || (C = !0)) : le(e)
    }), t(document, "keyup", function (t) {
        null != t.which || null == t.charCode && null == t.keyCode || (t.which = null != t.charCode ? t.charCode : t.keyCode), 13 === (ie = t.which || t.keyCode || 0) && e.isText ? X.onReturnKeyPressed() : 8 != ie && 46 != ie ? C && 84 === ie && e.isText ? X.showTextTools() : (C && 90 === ie && o.length && (o.length = o.length - 1, b.redraw(), de(!(!e.isDragAllPaths && !e.isDragLastPath))), C && 65 === ie && (y.global.startingIndex = 0, v(), te(n("drag-all-paths"), "DragAllPaths")), C && 67 === ie && o.length && I(), C && 86 === ie && B.length && (P(), de(!(!e.isDragAllPaths && !e.isDragLastPath))), void 0 !== t.metaKey && !1 === t.metaKey && (C = !1, ie = 17), 17 === ie && (C = !1)) : le(t) && X.writeText(X.lastKeyPress, !0)
    }), t(document, "keypress", function (e) {
        null != e.which || null == e.charCode && null == e.keyCode || (e.which = null != e.charCode ? e.charCode : e.keyCode), ie = e.which || e.keyCode || 0;
        var t = String.fromCharCode(ie);
        /[a-zA-Z0-9-_ !?|\/'",.=:;(){}\[\]`~@#$%^&*+-]/.test(t) && X.writeText(String.fromCharCode(ie))
    }), t(document, "paste", function (t) {
        if (e.isText) {
            var n = void 0;
            window.clipboardData && window.clipboardData.getData ? n = window.clipboardData.getData("Text") : t.clipboardData && t.clipboardData.getData && (n = t.clipboardData.getData("text/plain")), n && n.length && X.writeText(n)
        }
    });
    var ge, ce = 0;

    function de(e) {
        if (e && (ce = 0), ce != o.length) {
            for (var t, n = [], A = ce; A < o.length; A++) n[A - ce] = o[A];
            n.length && (t = {
                points: n || [],
                startIndex: ce
            }, window.parent.postMessage({
                canvasDesignerSyncData: t,
                uid: ge
            }, "*")), !n.length && o.length || (ce = o.length)
        }
    }
    window.addEventListener("message", function (e) {
        if (e.data)
            if (ge || (ge = e.data.uid), e.data.captureStream) fe.createOffer(function (e) {
                e.uid = ge, window.parent.postMessage(e, "*")
            });
            else if (e.data.renderStream) Ae();
        else if (e.data.sdp) fe.setRemoteDescription(e.data);
        else if (e.data.genDataURL) {
            var t = p.canvas.toDataURL(e.data.format, 1);
            window.parent.postMessage({
                dataURL: t,
                uid: ge
            }, "*")
        } else if (e.data.undo && o.length) {
            var n = e.data.index;
            if (e.data.tool) {
                for (var A = [], i = o.length, a = o.reverse(), s = 0; s < i; s++) {
                    (l = a[s])[0] !== e.data.tool && A.push(l)
                }
                return o = A.reverse(), b.redraw(), void de(!0)
            }
            if ("all" === n) return o = [], b.redraw(), void de(!0);
            if (n.numberOfLastShapes) {
                try {
                    o.length -= n.numberOfLastShapes
                } catch (e) {
                    o = []
                }
                return b.redraw(), void de(!0)
            }
            if (-1 === n) {
                if (o.length && "pencil" === o[o.length - 1][0]) {
                    var r;
                    for (A = [], i = o.length, a = o.reverse(), s = 0; s < i; s++) {
                        var l;
                        "start" == (l = a[s])[3] ? r = !0: r && A.push(l)
                    }
                    return o = A.reverse(), b.redraw(), void de(!0)
                }
                return o.length = o.length - 1, b.redraw(), void de(!0)
            }
            if (o[n]) {
                var g = [];
                for (s = 0; s < o.length; s++) s !== n && g.push(o[s]);
                o = g, b.redraw(), de(!0)
            }
        } else if (e.data.syncPoints) de(!0);
        else {
            if (e.data.clearCanvas) return o = [], void b.redraw();
            if (e.data.canvasDesignerSyncData) {
                var c = e.data.canvasDesignerSyncData;
                if (0 !== c.startIndex)
                    for (s = 0; s < c.points.length; s++) o[s + c.startIndex] = c.points[s];
                else o = c.points;
                ce = o.length, b.redraw()
            }
        }
    }, !1);
    var fe = {
        createOffer: function (e) {
            var t = document.getElementById("main-canvas").captureStream(),
                n = this.getPeer();
            t.getTracks().forEach(function (e) {
                n.addTrack(e, t)
            }), n.onicecandidate = function (t) {
                t && !t.candidate && e({
                    sdp: n.localDescription.sdp,
                    type: n.localDescription.type
                })
            }, n.createOffer({
                OfferToReceiveAudio: !1,
                OfferToReceiveVideo: !1
            }).then(function (e) {
                n.setLocalDescription(e)
            })
        },
        setRemoteDescription: function (e) {
            this.peer.setRemoteDescription(new RTCSessionDescription(e)).then(function () {
                Ae()
            })
        },
        createAnswer: function (e, t) {
            var n = this.getPeer();
            n.onicecandidate = function (e) {
                e && !e.candidate && t({
                    sdp: n.localDescription.sdp,
                    type: n.localDescription.type
                })
            }, this.peer.setRemoteDescription(new RTCSessionDescription(e)).then(function () {
                n.createAnswer({
                    OfferToReceiveAudio: !1,
                    OfferToReceiveVideo: !0
                }).then(function (e) {
                    n.setLocalDescription(e)
                })
            }), n.ontrack = function (e) {
                t({
                    stream: e.streams[0]
                })
            }
        },
        getPeer: function () {
            var e = window.RTCPeerConnection || window.webkitRTCPeerConnection || window.mozRTCPeerConnection,
                t = (window.RTCSessionDescription || window.mozRTCSessionDescription, window.RTCIceCandidate || window.mozRTCIceCandidate, new e(null));
            return this.peer = t, t
        }
    }
}();
